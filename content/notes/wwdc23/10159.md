...
New `safeAreaPadding` adds padding to the safe area
...
Conditionalize the count based on the horizontal sizeClass - two columns on ipad and one column on iphone
...
`aspectRatio` to have a height relative to the width, instead of hardcoding a fixed height
...
Scale view down in size when it gets near the edges of the ScrollView
